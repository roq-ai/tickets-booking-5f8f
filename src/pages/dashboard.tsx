...
import { getTravelAgents } from 'apiSdk/travel-agents';
...
function DashboardPage() {
  ...
  const travelAgentStat = useSWR<PaginatedInterface<TravelAgentInterface>>(
    () => `/travel-agents`,
    () => getTravelAgents({ limit: 1, offset: 0 }),
  );
  ...
  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Travel Agents',
      result: travelAgentStat?.data?.totalCount,
      isLoading: travelAgentStat.isLoading,
